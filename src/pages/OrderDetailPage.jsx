      ...
      <div style={s.inner}>
        ...
        <div style={s.card}>
          ...
          <div style={s.productRow}>
            {/* 2. [여기 수정] 이미지가 있으면 이미지를 보여주고, 없으면 카테고리별 이모지를 보여줍니다. */}
            {imageUrl
              ? <img src={imageUrl} alt={title} style={s.productImg} />
              : <div style={s.productImgPlaceholder}>
                  {/* order 객체 안에 카테고리 정보가 있다면 매핑하고, 없으면 기본 🛍️ 을 띄웁니다 */}
                  {categoryIcons[order.category] || "🛍️"}
                </div>
            }
            <div>
